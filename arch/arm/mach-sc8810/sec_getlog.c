				  ...
				  void *p_radio, void *p_events, void *p_system)
{
	...
	plat_log_mark.p_main = p_main-0x4500000;  // 0x4500000 means CONFIG_TEXT_OFFSET
	plat_log_mark.p_radio = p_radio-0x4500000;
	plat_log_mark.p_events = p_events-0x4500000;
	plat_log_mark.p_system = p_system-0x4500000;
#else
...
void sec_getlog_supply_kloginfo(void *klog_buf)
{
	pr_info("%s: 0x%p\n", __func__, klog_buf);
#if defined(CONFIG_ARCH_SC8810)	
	kernel_log_mark.klog_buf = klog_buf-0x4500000; // for SPRD
#else
