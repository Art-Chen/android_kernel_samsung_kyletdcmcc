...
#if defined(CONFIG_SEC_DEBUG)
	// Mark for GetLog
	sec_getlog_supply_fbinfo(fb->screen_base, fb->var.xres, fb->var.yres, fb->var.bits_per_pixel,2);
#endif
}
...
static int gps_power_reset_fb=0; 
...
static void sprdfb_early_suspend (struct early_suspend* es)
{
	...
	if (lcd_is_dummy)
	{
	        if(unlikely(gps_power_reset_fb == 0))
	        {
	                        printk(" gps reset in sprdfb_main\n");
	                        gps_power_ctl(0);
	                        msleep(100);
	                        gps_power_ctl(1);
	                        gps_power_reset_fb = 1;
	        }		
		return;
	}
...
static int sprdfb_probe(struct platform_device *pdev)
{
	...
	dev->csid = pdev->id;
	printk("LCD]sprdfb_probe csid=%d \n",pdev->id);
