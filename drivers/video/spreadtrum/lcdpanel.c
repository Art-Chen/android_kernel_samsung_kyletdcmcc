...
static int mcu_mount_panel(struct sprdfb_device *dev, struct panel_spec *panel)
{
	...
	dev->panel = panel;
	printk(KERN_INFO "LCD] mcu_mount_panel  ============panel=%x\n",panel);
...
static struct panel_spec *adapt_panel_from_uboot(int cs)
{
	...
	printk(KERN_INFO "LCD] adapt_panel_from_uboot   =============!\n");
...
static struct panel_spec *adapt_panel_from_readid(struct sprdfb_device *dev, int cs)
{
	...
	printk(KERN_INFO "LCD] adapt_panel_from_readid  id=%x  cfg->lcd_id=%x!\n",id, cfg->lcd_id);

		if(id == cfg->lcd_id) {
			printk(KERN_INFO "LCD Panel 0x%x is attached!\n", cfg->lcd_id);
			return cfg->panel;
		}
	//printk(KERN_INFO "LCD] adapt_panel_from_readid  2 ============\n");
	return NULL;		
	}
...
#else
	panel = adapt_panel_from_readid(dev, cs);
	printk(KERN_ERR "sprd_probe_panel panel=%x\n",panel);
	if (panel != NULL) {
		dev->mount_panel(dev, panel);
		dev->init_panel(dev);
		...
		/* can not be here in normal; we get correct device id from uboot */
		// temp block mcu_reset(NULL); /* hardware reset , Need not ? */
		//panel = adapt_panel_from_readid(dev, cs);
		//return 1; //error
        // Aijun Patch
