...
static int plugin_callback(int usb_cable, void *data)
{
	...
	if (usb_cable) {
printk("plugin_callback usb_cable\n");		
		d->usb_online = 1;
#if defined(CONFIG_SPA)
...
printk("plugin_callback ac_cable\n");					
			d->ac_online = 1;
#if defined(CONFIG_SPA)
			if(spa_external_event)
			{
...
printk("plugin_callback unkown_cable\n");			
			d->usb_online = 1;
#if defined(CONFIG_SPA)
			if(spa_external_event)
			{
...
static int detect_device()
{
	...
	ret = gpio_request(GPIO_JIG_ON, "jigon");
	if (ret) 
		printk("detect_device fail to request gpio %d\n", ret);
	
	gpio_direction_input(GPIO_JIG_ON);
	
	jig_on = gpio_get_value(GPIO_JIG_ON);
