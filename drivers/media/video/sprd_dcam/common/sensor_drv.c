...
int32_t Sensor_regs_table_init(void)
{
	...
	filp = filp_open("/mnt/extSdCard/sensor_s5k5ccgx_regs.h", O_RDONLY, 0);
	if (IS_ERR(filp)) {
...
void Sensor_QReset(uint32_t level)
{
	...
	if (PNULL != reset_func) {
		...
		err = gpio_request(GPIO_SENSOR_RESET, "ccirrst");
		if (err) {
			printk("Sensor_QReset failed requesting err=%d\n", err);
			return;
		}
		gpio_direction_output(GPIO_SENSOR_RESET, level);
		gpio_set_value(GPIO_SENSOR_RESET, level);
...
BOOLEAN Sensor_SetResetLevel(BOOLEAN plus_level)
{
	...
	err = gpio_request(GPIO_SENSOR_RESET, "ccirrst");
	if (err) {
		printk("Sensor_SetResetLevel failed requesting err=%d\n", err);
		return SENSOR_FAIL;
	}
...
LOCAL uint32_t _Sensor_Register(SENSOR_ID_E sensor_id)
{
	...
	SENSOR_PRINT_HIGH(" _Sensor_Register %d \n", sensor_id);
...
uint32_t Sensor_Init(uint32_t sensor_id)
{
	...
	do_gettimeofday(&time2);
	SENSOR_PRINT("SENSOR: Sensor_Init time=%d.\n",((time2.tv_sec-time1.tv_sec)*1000+(time2.tv_usec-time1.tv_usec)/1000));

	printk("Sensor_Init: end init: %d \n", ret_val);
	return ret_val;
}
...
ERR_SENSOR_E Sensor_SetMode(SENSOR_MODE_E mode)
{
	...
	if (result > 0)
	{		
		SENSOR_PRINT(KERN_ERR "***** s5k5ccgx_regs_table_init  FAILED. Check the Filie in MMC\n");
		return result;
	}
	result =0;
            ...
            if(SENSOR_MODE_COMMON_INIT == mode)
				...
				Sensor_regs_table_write("s5k5ccgx_YUV_640X480");
    ...
    do_gettimeofday(&time2);
    printk("SENSOR: Sensor_SetMode time=%d.\n",((time2.tv_sec-time1.tv_sec)*1000+(time2.tv_usec-time1.tv_usec)/1000));
