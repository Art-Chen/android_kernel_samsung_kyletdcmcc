...
uint32_t g_zoom_level = 3;	/*zoom level: 0: 1x, 1: 2x, 2: 3x, 3: 4x */
...
void reset_sensor_param(uint32_t capture_mode)
{
	/* Setting sensor parameters */
	if (INVALID_VALUE != g_dcam_info.wb_param)
		Sensor_Ioctl(SENSOR_IOCTL_SET_WB_MODE, g_dcam_info.wb_param);
	if (INVALID_VALUE != g_dcam_info.metering_param)
		Sensor_Ioctl(SENSOR_IOCTL_SET_MERTERING_MODE, g_dcam_info.metering_param);
	...
	if (1 != capture_mode) {
		...
		if (INVALID_VALUE !=  g_dcam_info.fps_param){
		Sensor_Ioctl(SENSOR_IOCTL_SET_FPS, g_dcam_info.fps_param);
		}
...
int Sensor_SetModeEX(uint32_t width, uint32_t height, uint32_t capture_mode)
{
	    ...
	    && (1 == capture_mode)) 
	{
		...
		DCAM_V4L2_PRINT("V4L2: sensor_ioctl_before_snapshot: = %d\n",size_mode);		
...
static int init_sensor_parameters(void *priv)
{
	...
	printk("V4L2:init_sensor_parameters  width = %d,height = %d .\n ",
				  fh->width,fh->height);

	for (i = SENSOR_MODE_PREVIEW_ONE; i < SENSOR_MODE_MAX; i++) {
		width = sensor_info_ptr->sensor_mode_info[i].width;
		//printk("input=%d, width=%d, i=%d \n", init_param.input_size.w, width, i);
	...
	printk("init_sensor_parameters: preview_m=%d, snapshot_m=%d \n",
	...
	/* Setting sensor parameters */
	reset_sensor_param(dev->streamparm.parm.capture.capturemode);
	return 0;
}
				...
				struct v4l2_format *f)
{
	...
	DCAM_V4L2_PRINT("V4L2: vidioc_g_fmt_vid_cap 1: w=%d,h=%d .\n",f->fmt.pix.width, f->fmt.pix.height);	
	return (0);
}
				...
				struct v4l2_format *f)
{
	...
	DCAM_V4L2_PRINT("V4L2: vidioc_s_fmt_vid_cap 1: w=%d,h=%d .\n",f->fmt.pix.width, f->fmt.pix.height);
			    ...
			    struct v4l2_querymenu *qm)
{
	...
	pr_debug("DCAM V4L2:vidioc_querymenu,addr=0x%x,size=0x%x, actuall_size=%x \n", 
		 qm->id, qm->index, qm->index);
...
*/
	...
	if (1 == init_param.mode) {
		...
		init_param.input_rect.w = g_dcam_info.input_size.w = fh->width;
		init_param.input_rect.h = g_dcam_info.input_size.h = fh->height;
	...
	while (1) {
		...
		case DCAM_NO_RUN:
			...
			if (DCAM_MODE_TYPE_PREVIEW == info_ptr->mode) {
				...
				Sensor_SetModeEX(g_dcam_info.input_size.w, g_dcam_info.input_size.h, 0);
				reset_sensor_param(0);
			} else {
				Sensor_SetTiming(g_dcam_info.snapshot_m);
				Sensor_SetModeEX(g_dcam_info.input_size.w, g_dcam_info.input_size.h, 1);
				reset_sensor_param(1);
			}
