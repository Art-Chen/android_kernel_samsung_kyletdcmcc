	...
	struct snd_pcm_hw_params *params)
{
	...
	if (revert_channel_da) {
		...
		dma_buff_phys = runtime->dma_addr + rtd->dma_da_ad_1_offset;
		/* channel 2 dma start addr */
		dma_buff_phys1= runtime->dma_addr;
	} else {
	/* channel 1 dma start addr */
	dma_buff_phys = runtime->dma_addr;
	/* channel 2 dma start addr */
	dma_buff_phys1= dma_buff_phys;
	}
...
#define FIX_ALSA_LIB_NXP_PRECISION 1
#if FIX_ALSA_LIB_NXP_PRECISION
/*
 * In function snd_pcm_update_hw_ptr0(), because of OS-vlx, OS-RTOS and OS-linux_kernel
 * has preempt action, OS-linux_kernel sometimes is preempted by OS-vlx and OS-RTOS,
 * audio dma data transfered pointer always not match with the timestamp get from snd_pcm_update_hw_ptr0()
 * snd_pcm_gettime(runtime, (struct timespec *)&runtime->status->tstamp);
 * so we need fix this alsa-lib bug, actually we need change
 * .pointer(struct snd_pcm_substream *substream) to
 * .pointer(struct snd_pcm_substream *substream, struct timespec *tstamp);
 * but it will need change so much other code not controled by sprd,
 * so we use global variable named struct timespec timestamp_match_with_dma_ptr_precision
 * to fix this alsa-lib bug [luther - 20120914]
 */
struct timespec timestamp_match_with_dma_ptr_precision;
EXPORT_SYMBOL_GPL(timestamp_match_with_dma_ptr_precision);
#ifdef CONFIG_NKERNEL
#define vlx_os_linux_lock() \
		flags = hw_local_irq_save()
#define vlx_os_linux_unlock() \
...
sc88xx_pcm_pointer(struct snd_pcm_substream *substream)
{
...
#if FIX_ALSA_LIB_NXP_PRECISION
	static int vbc_holded_dma_ptr[SNDRV_PCM_STREAM_LAST];
	unsigned long flags;
	int tstamp_mode = (runtime->tstamp_mode == SNDRV_PCM_TSTAMP_ENABLE) ? 1 : 0;
	...
	if (dma) {
		...
		if (revert_channel_da)
			data_base += rtd->dma_da_ad_1_offset;
...
#if FIX_ALSA_LIB_NXP_PRECISION
		if (tstamp_mode) {
			snd_pcm_gettime(runtime, &timestamp_match_with_dma_ptr_precision);
			vlx_os_linux_unlock();
		}
#endif
		free_data_height = (ptr - data_base) * channels; /* Each channel data transfer is symmetrical */
#if FIX_ALSA_LIB_NXP_PRECISION
		vbc_holded_dma_ptr[substream->stream] = free_data_height;
	} else {
		free_data_height = vbc_holded_dma_ptr[substream->stream];
	}
#else
	}
#endif
